...
const StatsScreen = ({ transactions }) => {
    ...
    const filteredTx = useMemo(() => {
        ...
        return transactions.filter((t) => {
            // 1. Convert the database string (e.g., 'Jan 17, 2026') to a Date object
            const dbDate = new Date(t.date);

            // 2. Convert the input filter string (e.g., '2026-01-17') to a Date object
            const filterDate = new Date(dateFilter);

            // 3. Compare using 'toDateString' to ignore time and focus only on the day
            return dbDate.toDateString() === filterDate.toDateString();
        });
    }, [transactions, dateFilter]);

    const expenseTx = filteredTx.filter((t) => t.isIncome === 0 || t.isIncome === false);
